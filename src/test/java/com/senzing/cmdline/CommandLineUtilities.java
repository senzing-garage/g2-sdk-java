...
public class CommandLineUtilities {
  /**
   * The value to use in the JSON representation of the parsed command-line parameters when a
   * {@linkplain CommandLineOption#isSensitive() sensitive} option is encountered.
   */
  ...
  /**
   * Returns a new {@link String} array that contains the same elements as the specified array
   * except for the first N arguments where N is the specified count.
   *
   ...
   * @throws TooFewArgumentsException If there are not enough arguments for the shift.
   */
  ...
  /**
   * Stores the option and its value(s) in the specified option map, first checking to ensure that
   * the option is NOT already specified and checking if the option has any conflicts. If the
   * specified option is deprecated then a warning message is printed.
   *
   ...
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
   * @throws CommandLineException If the specified {@link CommandLineValue} cannot be put in the
   *     specified {@link Map} due to a validation problem.
   */
  public static <
          T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>>
      void putValue(Map<CommandLineOption, CommandLineValue> optionMap, CommandLineValue value)
  ...
  /**
   * Checks if the specified {@link CommandLineValue} is a zero or single-valued boolean flag and
   * the value is false.
   *
  ...
  /**
   * Returns the enumerated {@link CommandLineOption} value associated with the specified command
   * line flag and enumerated {@link CommandLineOption} class.
   *
   * @param enumClass The {@link Class} for the {@link CommandLineOption} implementation.
   * @param commandLineFlag The command line flag.
   * @return The enumerated {@link CommandLineOption} or <code>null</code> if not found.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
   ...
   *
   * @param enumClass The enum class for the {@link CommandLineOption} implementation.
   * @return <code>true</code> if a primary option is required, otherwise <code>false</code>.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
  ...
  /**
   * Gets the ordered {@link Set} describing the type chain for the specified command line option
   * class.
   *
  ...
  /**
   * Populates the type chain for the specified command line option class in the specified {@link
   * Set}.
   *
  ...
  private static <
          T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>>
      void populateTypeChain(Set<Class<? extends CommandLineOption>> set, Class<T> enumClass) {
  ...
  /**
   * Populates the options chain for the specified command line option class in the specified {@link
   * Set}.
   *
   * @param set The {@link} set to populate.
  ...
  private static <
          T extends Enum<T> & CommandLineOption<T, B>, B extends Enum<B> & CommandLineOption<B, ?>>
      void populateOptionsChain(Set<CommandLineOption> set, Class<T> enumClass) {
    if (enumClass == null) return;
    EnumSet<T> enumSet = EnumSet.allOf(enumClass);
    Class baseType = null;
  ...
  /**
   * Validates the specified {@link Set} of specified {@link CommandLineOption} instances and
   * ensures that they logically make sense together. This checks for the existing of at least one
   * primary option (if primary options exist), ensures there are no conflicts and that all
   * dependencies are satisfied.
   *
   * @param enumClass The {@link Class} object identifying the enumerated type that implements
   *     {@link CommandLineOption}.
   * @param optionValues The {@link Map} of {@link CommandLineOption} keys to {@link
   *     CommandLineValue} values.
   * @return A {@link List} of {@link DeprecatedOptionWarning} instances describing the deprecation
   *     warnings (if any), or <code>null</code> if there are no deprecation warnings.
   * @throws IllegalArgumentException If the specified options are invalid together.
   * @throws CommandLineException If the specified command-line options fail validation.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
  ...
  public static <
      ...
      List<DeprecatedOptionWarning> validateOptions(
          Class<T> enumClass, Map<CommandLineOption, CommandLineValue> optionValues)
  ...
  /**
   * Creates a lookup map of {@link String} command-line flags to the {@link CommandLineOption}
   * values that they map to. This includes any synonyms for the options if they exist.
   *
  ...
  /**
   * Parses the command line arguments and returns a {@link Map} of those arguments. This function
   * will attempt to find values for command-line options that are not explicitly specified on the
   * command line by using environment variables for those options have environment variable
   * equivalents defined.
   *
   * @param enumClass The enumerated {@link CommandLineOption} class.
   * @param args The arguments to parse.
   * @param processor The {@link ParameterProcessor} to use for handling the parameters to the
   *     options.
   * @param deprecationWarnings The {@link List} to be populated with any {@link
   *     DeprecatedOptionWarning} instances that are generated, or <code>null</code> if the caller
   *     is not interested in deprecation warnings.
  ...
  public static <
      ...
      Map<CommandLineOption, CommandLineValue> parseCommandLine(
          ...
          throws CommandLineException {
    return parseCommandLine(enumClass, args, processor, false, null, deprecationWarnings);
  }

  /**
   * Parses the command line arguments and returns a {@link Map} of those arguments. Depending on
   * the specified value for <code>ignoreEnvironment</code> this function will optionally attempt to
   * find values for command-line options that are not explicitly specified on the command line by
   * using environment variables for those options have environment variable equivalents defined.
   *
   * @param enumClass The enumerated {@link CommandLineOption} class.
   * @param args The arguments to parse.
   * @param processor The {@link ParameterProcessor} to use for handling the parameters to the
   *     options.
   * @param ignoreEnvironment Flag indicating if the environment variables should be ignored in the
   *     processing.
   * @param deprecationWarnings The {@link List} to be populated with any {@link
   *     DeprecatedOptionWarning} instances that are generated, or <code>null</code> if the caller
   *     is not interested in deprecation warnings.
  ...
  /**
   * Parses the command line arguments and returns a {@link Map} of those arguments. This function
   * will optionally attempt to find values for command-line options that are not explicitly
   * specified on the command line by using environment variables for those options have environment
   * variable equivalents defined. Use of the environment is disabled if the <code>ignoreEnvOption
   * </code> parameter is non-null and that option is present in the explicitly specified
   * command-line arguments and either has no parameter value or any value other than <code>false
   * </code>.
   *
   * @param enumClass The enumerated {@link CommandLineOption} class.
   * @param args The arguments to parse.
   * @param processor The {@link ParameterProcessor} to use for handling the parameters to the
   *     options.
   * @param ignoreEnvOption The optional command-line option value that if present with no value or
   *     present with any value other than <code>false</code> will cause the environment to be
  ...
  public static <
    ...
    return parseCommandLine(
        enumClass, args, processor, false, ignoreEnvOption, deprecationWarnings);
  }

  /**
   * Parses the command line arguments and returns a {@link Map} of those arguments.
   *
   * @param enumClass The enumerated {@link CommandLineOption} class.
   * @param args The arguments to parse.
   * @param processor The {@link ParameterProcessor} to use for handling the parameters to the
   ...
   *     DeprecatedOptionWarning} instances that are generated, or <code>null</code> if the caller
   *     is not interested in deprecation warnings.
   * @return The {@link Map} of {@link CommandLineOption} keys to {@link CommandLineValue} values
   *     that are the result of the parsing.
   * @throws CommandLineException If the specified command line arguments fail to parse.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
  ...
  private static <
    ...
    for (int index = 0; index < args.length; index++) {
      ...
      if (minParamCount > 0) {
        ...
        for (index++; index <= max; index++) {
          if (index >= args.length) {
            throw new BadOptionParameterCountException(COMMAND_LINE, option, flag, params);
          }
      ...
      if (minParamCount == 0
        ...
        int bound = (maxParamCount < 0) ? args.length : index + (maxParamCount - minParamCount) + 1;
      ...
      Object processedValue = processValue(COMMAND_LINE, option, flag, processor, params);

      // create the command-line value
      CommandLineValue cmdLineVal =
          new CommandLineValue(COMMAND_LINE, option, flag, processedValue, params);
   ...
   * @throws BadOptionParametersException If the parameter values cannot be processed without an
   *     error.
   */
   ...
   *
   * @param enumClass The {@link Class} for the enumerated type that implements {@link
   *     CommandLineOption}.
   * @param processor The {@link ParameterProcessor} to use.
   * @param optionValues The {@link Map} of {@link CommandLineOption} keys to {@link
   *     CommandLineValue} values.
   * @throws CommandLineException If a command-line option processing error occurs.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
   * @param <B> The base enumerated type that the command-line options extend, <b>OR</b> the same as
   *     type <code>T</code> if the command-line option type has no base and returns <code>null
   *     </code> from {@link CommandLineOption#getBaseOptionType()}.
   ...
   *
   * @throws CommandLineException If the specified environment command line arguments are illegal.
   */
    ...
    for (CommandLineOption option : enumSet) {
      ...
      if (paramCount != 1 || minParamCount != 0 || maxParamCount != 0) {
        ...
        if (paramCount < minParamCount) {
          throw new BadOptionParameterCountException(ENVIRONMENT, option, foundVar, params);
        }
      ...
      // process the parameters
      Object processedValue = processValue(ENVIRONMENT, option, foundVar, processor, params);

      // create the command line value
      CommandLineValue cmdLineVal =
          new CommandLineValue(ENVIRONMENT, option, foundVar, processedValue, params);
   ...
   * Check for command-line option values in the environment.
   *
   * @param enumClass The option enum class that implements {@link CommandLineOption}.
   * @param processor The {@link ParameterProcessor} for processing the parameters.
   * @param optionValues The {@link Map} of {@link CommandLineOption} keys to {@link
   *     CommandLineValue} values.
   * @throws CommandLineException If a command-line option processing error occurs.
   * @param <T> The enumerated type that implements {@link CommandLineOption}.
  ...
  private static <
    ...
    for (CommandLineOption option : options) {
      ...
      // process the parameters
      Object processedValue = processValue(DEFAULT, option, null, processor, params);

      // create the command line value
      CommandLineValue cmdLineVal = new CommandLineValue(DEFAULT, option, processedValue, params);
  ...
  /**
   * Processes the {@link Map} of {@link CommandLineOption} keys to {@link CommandLineValue} values
   * and produces the {@link Map} of {@link CommandLineOption} keys to {@linkplain
   * CommandLineValue#getProcessedValue() processed values}.
   *
   * @param optionValues The {@link Map} of option keys to {@link CommandLineValue} values.
   * @param processedValues The {@link Map} to populate with the processed values if specified, if
   *     <code>null</code> a new {@link Map} is created and returned.
   * @return The {@link Map} of {@link CommandLineOption} keys to {@link Object} values describing
   ...
   * @param processedValues The {@link Map} to populate with the processed values if specified, if
   *     <code>null</code> a new {@link Map} is created and returned.
   * @param jsonBuilder If not <code>null</code> then this {@link JsonObjectBuilder} is populated
   *     with startup option information.
   * @return The {@link Map} of {@link CommandLineOption} keys to {@link Object} values describing
   *     the parameters for the option.
   */
  public static Map<CommandLineOption, Object> processCommandLine(
    ...
    return processCommandLine(optionValues, processedValues, jsonBuilder, null);
  }
   ...
   * @param processedValues The {@link Map} to populate with the processed values if specified, if
   *     <code>null</code> a new {@link Map} is created and returned.
   * @param stringBuilder If not <code>null</code>then this {@link StringBuilder} is populated with
   *     JSON text describing the startup options.
   * @return The {@link Map} of {@link CommandLineOption} keys to {@link Object} values describing
   *     the parameters for the option.
   */
  public static Map<CommandLineOption, Object> processCommandLine(
    ...
    return processCommandLine(optionValues, processedValues, null, stringBuilder);
  }
  ...
  /**
   * Checks if the specified {@link Class} was the class whose static <code>main(String[])</code>
   * function was called to begin execution of the current process.
   *
   * @param cls The {@link Class} to test for.
   * @return <code>true</code> if the specified class' static <code>main(String[])</code> function
   *     was called to begin execution of the current process.
   */
  ...
  /**
   * Returns a multi-line bulleted list of the specified options with the specified indentation.
   *
  ...
  /**
   * Gets the base URL for the JAR file where the specified {@link Class} is found. This method
   * returns <code>null</code> if the JAR base URL cannot be determined.
   *
   * @param cls The {@link Class} for which the JAR base URL is requested.
   * @return The base URL for the JAR that has the specified class, or <code>null</code> if it
  ...
  /**
   * Gets the name of the JAR file that contains the specified {@link Class}. This method returns
   * <code>null</code> if the JAR file name cannot be determined.
   *
   * @param cls The {@link Class} for which the jar file name is requested.
   * @return The name of the JAR file that has the specified class, or <code>null</code> if it
   *     cannot be determined.
   */
  ...
  /**
   * Gets the path of the JAR file that contains the specified {@link Class}. This method returns
   * <code>null</code> if the JAR file path cannot be determined.
   *
   * @param cls The {@link Class} for which the jar file path is requested.
   * @return The path to the JAR file that has the specified class, or <code>null</code> if it
   *     cannot be determined.
   */
