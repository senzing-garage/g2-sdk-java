...
public class LoggingUtilities {
  ...
  /**
   * The {@link DateTimeFormatter} for interpretting the build number as a LocalDateTime instance.
   */
  ...
  /**
   * Sets the product ID to use when logging messages for classes in the specified package name.
   *
  ...
  /**
   * Gets the product ID for the package name of the class performing the logging.
   *
   * @param packageName The package name.
  ...
  public static String getProductIdForPackage(String packageName) {
    synchronized (PRODUCT_ID_MAP) {
      do {
        ...
        if (packageName.startsWith("com.senzing.") && packageName.length() > prefixLength) {
   ...
   *
   * @return <code>true</code> if debug logging is enabled, and <code>false</code> if not.
   */
  ...
  /**
   * Produces a single or multi-line error log message with a consistent prefix and timestamp.
   *
   * @param lines The lines of text to log, which may be objects that will be converted to text via
  ...
  /**
   * Produces a multi-line error log message with a consistent prefix and timestamp that will
   * conclude with the stack trace for the specified {@link Throwable}.
   *
  ...
  /**
   * Produces a single or multi-line warning log message with a consistent prefix and timestamp.
   *
   * @param lines The lines of text to log, which may be objects that will be converted to text via
   *     {@link Object#toString()}.
   */
  ...
  /**
   * Produces a multi-line warning log message with a consistent prefix and timestamp that will
   * conclude with the stack trace for the specified {@link Throwable}.
   *
   * @param throwable The {@link Throwable} whose stack trace should be logged.
   * @param lines The lines of text to log, which may be objects that will be converted to text via
  ...
  /**
   * Produces a single or multi-line debug message with a consistent prefix and timestamp IF
   * {@linkplain #isDebugLogging() debug logging} is turned on.
   *
   * @param lines The lines of text to log, which may be objects that will be converted to text via
   *     {@link Object#toString()}.
   */
   ...
   *
   * @param lines The lines of text to log, which may be objects that will be converted to text via
   *     {@link Object#toString()}. @Deprecated Use {@link #logDebug(Object...)} instead.
   * @see #logDebug(Object...)
   */
  ...
  /**
   * Produces a single or multi-line log message with a consistent prefix and timestamp using the
   * specified {@link PrintStream}, {@link Object} to synchronize on, {@link String} log type and
   * lines of text to log.
   *
   * @param ps The {@link PrintStream} to write to.
   * @param monitor The {@link Object} to synchronize on when writing and flushing the {@link
   *     PrintStream}.
   * @param logType The logging type such as <code>"DEBUG"</code> or <code>"ERROR"</code>.
   * @param lines The lines of text to log.
   * @param throwable The exception to log.
   */
  private static void log(
      PrintStream ps, Object monitor, String logType, Object[] lines, Throwable throwable) {
   ...
   * @return The formatted {@link String} describing the array of {@link StackTraceElement}
   *     instances or <code>null</code> if the specified array is <code>null</code>.
   */
  ...
  /**
   * Formats a single {@link StackTraceElement} in the same format as they would appear in an
  ...
  /**
   * Formats an error message from a {@link NativeApi} instance, including the details (which may
   * contain sensitive PII information) as part of the result.
   *
   * @param operation The name of the operation from the native API interface that was attempted but
   *     failed.
   * @param fallible The {@link NativeApi} from which to extract the error message.
  ...
  /**
   * Formats an error message from a {@link NativeApi} instance, optionally including the details
   * (which may contain sensitive PII information) as part of the result.
   *
   * @param operation The name of the operation from the native API interface that was attempted but
   *     failed.
   * @param fallible The {@link NativeApi} from which to extract the error message.
   * @param includeDetails <code>true</code> to include the details of the failure failure in the
   *     resultant message, and <code>false</code> to exclude them (usually to avoid logging
   *     sensitive information).
   * @return The multi-line formatted log message.
   */
  public static String formatError(String operation, NativeApi fallible, boolean includeDetails) {
  ...
  /**
   * Logs an error message from a {@link NativeApi} instance, including the details (which may
   * contain sensitive PII information) as part of the result.
   *
   * @param operation The name of the operation from the native API interface that was attempted but
   *     failed.
   * @param fallible The {@link NativeApi} from which to extract the error message.
   */
  public static void logError(String operation, NativeApi fallible) {
    logError(operation, fallible, true);
  }

  /**
   * Logs an error message from a {@link NativeApi} instance, optionally including the details
   * (which may contain sensitive PII information) as part of the result.
   *
   * @param operation The name of the operation from the native API interface that was attempted but
   *     failed.
   ...
   *     sensitive information).
   */
  public static void logError(String operation, NativeApi fallible, boolean includeDetails) {
    String message = formatError(operation, fallible, includeDetails);
    System.err.println(message);
  }

  /**
   * Convert a throwable to a {@link Long} value so we don't keep a reference to what could be a
   * complex exception object.
   *
  ...
  /**
   * Checks if the specified {@link Throwable} is the last logged exception or if the class of
   * specified {@link Throwable} is {@link RuntimeException} and it has a {@linkplain
   * Throwable#getCause()} then if the cause is the last logged exception. This is handy for telling
   * if the exception has already been logged by a deeper level of the stack trace.
   *
   * @param t The {@link Throwable} to check.
   * @return <code>true</code> if it is the last logged exception, otherwise <code>false</code>.
   */
  ...
  /**
   * Sets the specified {@link Throwable} as the last logged exception. This is handy for telling if
   * the exception has already been logged by a deeper level of the stack trace.
   *
   ...
   * #isLastLoggedException(Throwable) last logged exception} and then rethrows it.
   *
