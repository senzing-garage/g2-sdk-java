...
/**
 * The package-protected implementation of {@link SzConfig} that works with the {@link
...
class SzCoreConfig implements SzConfig {
        ...
        () -> {
          ...
          if (returnCode != 0) {
            this.env.handleReturnCode(
                ...
                "SzCoreConfig()",
                paramsOf(
                    "instanceName", this.env.getInstanceName(),
  ...
  public long createConfig() throws SzException {
    return this.env.execute(
        () -> {
          ...
          if (returnCode != 0) {
            this.env.handleReturnCode(returnCode, this.nativeApi, "SzCoreConfig.createConfig()");
          }
  ...
  @Override
  public void addDataSource(long configHandle, String dataSourceCode) throws SzException {
    this.env.execute(
        () -> {
          ...
          // call the underlying C function
          int returnCode = this.nativeApi.addDataSource(configHandle, inputJson, sb);
          ...
          if (returnCode != 0) {
            this.env.handleReturnCode(
                ...
                paramsOf(
                    "configHandle", hexFormat(configHandle), "dataSourceCOde", dataSourceCode));
          }
  ...
  @Override
  public void deleteDataSource(long configHandle, String dataSourceCode) throws SzException {
    this.env.execute(
        () -> {
          // format the JSON for the JNI call
          String inputJson = "{\"DSRC_CODE\":" + jsonEscape(dataSourceCode) + "}";
          ...
          // call the underlying C function
          int returnCode = this.nativeApi.deleteDataSource(configHandle, inputJson);
