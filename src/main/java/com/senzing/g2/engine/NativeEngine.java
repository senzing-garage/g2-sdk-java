...
/**
 * Defines the Java interface to the G2 engine functions. The G2 engine functions primarily provide
 * means of working with identity data records, entities and their relationships.
 */
interface NativeEngine extends NativeApi {
  /**
   * The bitwise flag for export functionality to indicate that we should include "resolved"
   * relationships
   */
  long G2_EXPORT_INCLUDE_MULTI_RECORD_ENTITIES = (1L << 0);

  /**
   * The bitwise flag for export functionality to indicate that we should include "possibly same"
   * relationships
   */
  long G2_EXPORT_INCLUDE_POSSIBLY_SAME = (1L << 1);

  /**
   * The bitwise flag for export functionality to indicate that we should include "possibly related"
   * relationships
   */
  long G2_EXPORT_INCLUDE_POSSIBLY_RELATED = (1L << 2);

  /**
   * The bitwise flag for export functionality to indicate that we should include "name only"
   * relationships
   */
  long G2_EXPORT_INCLUDE_NAME_ONLY = (1L << 3);

  /**
   * The bitwise flag for export functionality to indicate that we should include "disclosed"
   * relationships
   */
  ...
  /**
   * The bitwise flag for find-path functionality to indicate that excluded entities are still
  ...
  /**
   * The bitwise flag for search functionality to indicate that we should include "resolved" match
   * level results
   */
  long G2_SEARCH_INCLUDE_RESOLVED = (G2_EXPORT_INCLUDE_MULTI_RECORD_ENTITIES);

  /**
   * The bitwise flag for search functionality to indicate that we should include "possibly same"
   * match level results
   */
  long G2_SEARCH_INCLUDE_POSSIBLY_SAME = (G2_EXPORT_INCLUDE_POSSIBLY_SAME);

  /**
   * The bitwise flag for search functionality to indicate that we should include "possibly related"
   * match level results
   */
  long G2_SEARCH_INCLUDE_POSSIBLY_RELATED = (G2_EXPORT_INCLUDE_POSSIBLY_RELATED);

  /**
   * The bitwise flag for search functionality to indicate that we should include "name only" match
   * level results
   */
  long G2_SEARCH_INCLUDE_NAME_ONLY = (G2_EXPORT_INCLUDE_NAME_ONLY);

  /**
   * The bitwise flag for search functionality to indicate that we should include all match level
   * results
   */
  ...
  long G2_EXPORT_DEFAULT_FLAGS = (G2_EXPORT_INCLUDE_ALL_ENTITIES | G2_ENTITY_DEFAULT_FLAGS);
  ...
  long G2_HOW_ENTITY_DEFAULT_FLAGS = (G2_INCLUDE_FEATURE_SCORES);

  /** The default recommended bitwise flag values for virtual-entity-analysis on entities */
  long G2_VIRTUAL_ENTITY_DEFAULT_FLAGS = (G2_ENTITY_DEFAULT_FLAGS);

  /**
   * The default recommended bitwise flag values for searching by attributes, returning all matching
   * entities
   */
  ...
  /**
   * The default recommended bitwise flag values for searching by attributes, returning only
  ...
  /**
   * The default recommended bitwise flag values for searching by attributes, returning minimal data
   * with all matches
   */
  long G2_SEARCH_BY_ATTRIBUTES_MINIMAL_ALL = (G2_SEARCH_INCLUDE_ALL_ENTITIES);

  /**
   * The default recommended bitwise flag values for searching by attributes, returning the minimal
   * data, and returning only the strongest matches
   */
  ...
  /**
   * Initializes the G2 Engine API with the specified module name, init parameters and flag
   * indicating verbose logging. If the <code>G2CONFIGFILE</code> init parameter is absent then the
   * default configuration from the repository is used.
   *
   ...
   * @param verboseLogging Enable diagnostic logging which will print a massive amount of
  ...
  /**
   * Initializes the G2 Engine object with the module name, initialization parameters, verbose
   * logging flag and a specific configuration ID identifying the configuration to use.
   *
   ...
   *
   * @return Zero (0) on success, negative-one (-1) if the engine is not initialized.
   */
  int destroy();
  ...
  /**
   * Loads the JSON record with the specified data source code and record ID. The specified JSON
   * data may contain the <code>DATA_SOURCE</code> and <code>RECORD_ID</code> elements, but, if so,
   * they must match the specified parameters.
   *
  ...
  /**
   * This method is used to add entity data into the system. This works similarly to the
   * G2_addRecord function, except that instead or requiring the recordID as an input parameter, the
   * recordID is generated internally and returned through the parameter list. A list of modified
   * resolved entities is also returned
   *
   * @param dataSourceCode The data source for the observation.
   * @param recordID The ID for the record. If specified, then it will be used, if empty string,
   *     then G2 will generate an ID
   * @param jsonData A JSON document containing the attribute information for the observation.
   * @param flags The flags to control how the operation is performed and specifically the content
   *     of the response JSON document.
   * @param response A {@link StringBuffer} for returning the response document. If an error
   *     occurred, an error response is stored here.
   * @return Zero (0) on success and non-zero on failure.
   */
  int addRecordWithInfo(
  ...
  /**
   * This method searches for entities that contain attribute information that are relevant to a set
   * of input search attributes.
   *
   * @param jsonData A JSON document containing the attribute information to search for
   * @param response A {@link StringBuffer} for returning the response document. If an error
   *     occurred, an error response is stored here.
   * @return Zero (0) on success and non-zero on failure.
   */
  ...
  /**
   * This method is used to retrieve information about a specific resolved entity. The information
   * is returned as a JSON document.
   *
   * <p>An entityID may be named ENTITY_ID, RESOLVED_ID, or RELATED_ID in the JSON or CSV function
   * output.
   *
   * @param entityID The resolved entity to retrieve information for
   * @param response A {@link StringBuffer} for returning the response document. If an error
   *     occurred, an error response is stored here.
   * @return Returns zero (0) for success. Returns negative-one (-1) if the response status
   *     indicates failure or the G2 module is not initialized. Returns negative-two (-2) if an
  ...
  /**
   * This method is used to retrieve information about the resolved entity containing a particular
   * observation record.
   *
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int getEntityByRecordID(
      String dataSourceCode, String recordID, long flags, StringBuffer response);

  /**
   * This method is used to find interesting entities close to a specific resolved entity. The
   * information is returned as a JSON document.
   *
  ...
  /**
   * This method is used to find interesting entities close to a specific resolved entity containing
   * a particular observation record.
   *
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int findInterestingEntitiesByRecordID(
      String dataSourceCode, String recordID, long flags, StringBuffer response);

  /**
   * This method is used to find a relationship path between entities that are identified by entity
   * ID.
   *
   ...
   * @param response The {@link StringBuffer} to write the JSON response document to.
   * @return Zero (0) on success and non-zero on failure.
   */
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int findPathByEntityID(
      long entityID1, long entityID2, int maxDegrees, long flags, StringBuffer response);

  /**
   * This method is used to find a relationship path between entities that are identified by the
   * data source code and record ID of records in each of the entities.
   *
  ...
  /**
   * This method is used to find a relationship path between two entities identified by their entity
   * ID's that excludes one or more entities, also identified by their entity ID's.
   *
   * <p>The excluded entities are identified by their entity ID's in a JSON document with the
   * following format:
   *
   * <pre>
   *   {
   ...
   * @param entityID2 The entity ID of the second entity.
   * @param maxDegrees The maximum number of degrees for the path search.
   * @param excludedEntities The JSON document identifying the excluded entities via their entity
   *     ID's.
   * @param response The {@link StringBuffer} to write the JSON response document to.
   * @return Zero (0) on success and non-zero on failure.
   */
  int findPathExcludingByEntityID(
      long entityID1,
      long entityID2,
      int maxDegrees,
   ...
   * source codes and record IDs of their composite records where that path excludes one or more
   * entities, also identified by the data sourec codes and record IDs of their composite records.
   *
   * <p>The excluded entities are identified by the data source codes and record ID's of their
   * composite records in a JSON document with the following format:
   *
   * <pre>
   *   {
   *     "ENTITIES": [
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int findPathExcludingByRecordID(
      String dataSourceCode1,
      String recordID1,
      String dataSourceCode2,
      String recordID2,
  ...
  /**
   * This method is used to find a relationship path between two entities identified by their entity
   * ID's. The path will exclude the one or more entities, also identified by the specified entity
   * ID's and will require that the path contains <b>at least one</b> of the data sources identified
   ...
   * </pre>
   *
   * <p>The required set of data sources are identified by their data source codes in a JSON
   ...
   * @param excludedEntities The JSON document identifying the excluded entities via their entity
   *     ID's.
   * @param requiredSources The JSON document identifying the data sources that must be included on
   *     the path.
   * @param response The {@link StringBuffer} to write the JSON response document to.
   * @return Zero (0) on success and non-zero on failure.
   */
  int findPathIncludingSourceByEntityID(
      long entityID1,
      long entityID2,
      int maxDegrees,
      String excludedEntities,
  ...
  /**
   * This method is used to find a relationship path between two entities identified by the data
   * source codes and record IDs of their composite records. THe path will exclude the one or more
   * entities also identified by the specified data source code and record ID pairs that identify
   * the composite records of the excluded entities and further will require the path contains <b>at
   * least one</b> of the data sources identified by the one or more specified data sources codes.
   *
   * <p>The excluded entities are identified by the data source codes and record ID's of their
   * composite records in a JSON document with the following format:
   *
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int findPathIncludingSourceByRecordID(
      String dataSourceCode1,
      String recordID1,
      String dataSourceCode2,
      String recordID2,
  ...
  /**
   * This method is used to find a network of entity relationships, surrounding the paths between a
   * set of entities. The entities are identified by their entity IDs.
   *
   * <p>The desired entities are identified by their entity ID's in a JSON document with the
   * following format:
   *
   * <pre>
   *   {
   ...
   * </pre>
   *
   * @param entityList The JSON document specififying the entity ID's of the desired entities.
  ...
  /**
   * This method is used to find a network of entity relationships, surrounding the paths between a
   * set of entities. The entities are identified by their composite records having the specified
   * data source code and record ID pairs.
   *
   * <p>The composite records af the desired entities are identified by the data source code and
   * record ID pairs in a JSON document with the following format:
   *
   * <pre>
   *   {
   *     "ENTITIES": [
   ...
   * </pre>
   *
   * @param recordList The JSON document containing the data source code and record ID pairs for the
   *     composite records of the desired entities.
   * @param maxDegrees The maximum number of degrees for the path search between the specified
   *     entities.
   * @param buildOutDegrees The number of relationship degrees to build out from each of the found
   *     entities.
   ...
   *
   * @param dataSourceCode The data source code for the composite record of the subject entity.
   * @param recordID The record ID for the composite record of the subject entity.
   * @param response The {@link StringBuffer} to write the JSON response document to.
   * @return Zero (0) on success and non-zero on failure.
   */
  ...
  /**
   * This is used to export entity data from known entities. This function returns an export-handle
   * that can be read from to get the export data in JSON format. The export-handle should be read
   * using the "G2_fetchNext" function, and closed when work is complete. Each output row contains
   * the exported entity data for a single resolved entity.
   *
   * @param flags A bit mask specifying control flags. The default and recommended value is
   *     "G2_EXPORT_DEFAULT_FLAGS".
   * @param exportHandle The {@link Result} object for storing the export handle.
   * @return Zero (0) on success and non-zero on failure.
   */
  ...
  /**
   * This is used to export entity data from known entities. This function returns an export-handle
   * that can be read from to get the export data in CSV format. The export-handle should be read
   * using the "G2_fetchNext" function, and closed when work is complete. The first output row
   * returned by the export-handle contains the JSON column headers as a string. Each following row
   * contains the exported entity data.
   *
   ...
   *     names.
   * @param flags A bit mask specifying other control flags. The default and recommended value is
   *     "G2_EXPORT_DEFAULT_FLAGS".
   * @param exportHandle The {@link Result} object for storing the export handle.
   ...
   * @param response The {@link StringBuffer} to write the next exported record to.
   * @return Zero (0) on success and non-zero on failure.
   */
   ...
   * @return Zero (0) on success and non-zero on failure.
   */
  int processRedoRecordWithInfo(String redoRecord, StringBuffer response);
