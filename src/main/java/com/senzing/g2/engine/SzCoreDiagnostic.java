...
/**
 * The package-protected implementation of {@link SzDiagnostic} that works with the {@link
...
public class SzCoreDiagnostic implements SzDiagnostic {
   ...
   */
  SzCoreDiagnostic(SzCoreEnvironment environment) throws IllegalStateException, SzException {
  ...
  public String getDatastoreInfo() throws SzException {
    return this.env.execute(
        () -> {
          ...
          if (returnCode != 0) {
            this.env.handleReturnCode(
                returnCode, this.nativeApi, CLASS_PREFIX + ".getDatastoreInfo()");
          }
  ...
  public void purgeRepository() throws SzException {
    this.env.execute(
        () -> {
          ...
          if (returnCode != 0) {
            this.env.handleReturnCode(
                returnCode, this.nativeApi, CLASS_PREFIX + ".purgeRepository()");
          }
